...
Object.keys(variants).map((variant) => {
  ...
  Object.keys(variants[variant]).map((color) => {
    palette[variant][color] = variants[variant][color].hex;
  });
});
var flavors = Object.keys(palette);
var colors = Object.keys(palette[flavors[0]]);
...
var generateColorCss = (defaultFlavor = "", prefix = false) => {
  const result = {};
  flavors.map((variant) => {
...
var generateOptions = (prefix = false) => {
  const result = {};
  colors.map((color) => {
...
var src_default = import_plugin.default.withOptions(
  (options) => {
    return ({ addBase }) => {
      addBase(generateColorCss(options == null ? void 0 : options.defaultFlavour, options == null ? void 0 : options.prefix));
    };
  },
  (options) => {
    const extendOption = {};
    colorConfigKeys.map((key) => {
      extendOption[key] = generateOptions(options == null ? void 0 : options.prefix);
    });
    const config = {
      content: [],
      theme: {
